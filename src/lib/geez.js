...
function parse(input) {
    ...
    function parse_varname() {
        var name = input.next()
        if (name.type != 'var') input.croak('ውህብ በመጠበቅ ላይ ::')
        return name.value
    }
...
Environment.prototype = {
    ...
    get: function (name) {
        ...
        throw new Error('ያልተሰየመ ስም ፦' + name)
    },
    ...
    switch (exp.type) {
        ...
        case 'assign':
            if (exp.left.type != 'var') {
                term.Write(
                    'መመድብ አልተቻለም ፦ ' + JSON.stringify(exp.left),
                    'jqconsole-error'
                )
                throw new Error('መመድብ አልተቻለም ፦ ' + JSON.stringify(exp.left))
            }
...
function apply_op(op, a, b) {
    function num(x) {
        if (typeof x != 'number') {
            term.Write('ቁጥር በመጠበቅ ላይ ግን የቀረበው ፦ ' + x + '\n', 'jqconsole-error')
            throw new Error('ቁጥር በመጠበቅ ላይ ግን የቀረበው ፦ ' + x)
        }
        return x
    }
    function div(x) {
        if (num(x) == 0) {
            ...
            throw new Error('ቁጥር በ 0 ማካፈል አይቻልም ::')
        }
        return x
    }
    switch (op) {
    ...
    term.Write('ከዋኝ መፈፀም አልተቻለም ።' + '\n', 'jqconsole-error')
    throw new Error('ከዋኝ መፈፀም አልተቻለም ። ' + op)
}
export {
    Environment,
    parse,
